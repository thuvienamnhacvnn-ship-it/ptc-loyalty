// TEMPORARY key-guarded delivery verify. Token from env, never returned/logged.
...
export async function GET(req: NextRequest) {
  ...
  if (test) {
    ...
    if (upJson.id) {
      ...
      const sendJson: { messages?: { id: string }[]; error?: unknown } = await send.json().catch(() => ({}));
      ...
      if (wamid) {
        ...
        if (biz) {
          await db.whatsAppMessageLog.create({
            data: {
              businessId: biz.id, kind: "TEST", status: "SENT", direction: "OUTBOUND",
              toPhone: to, language: lang, templateKey: name,
              idempotencyKey: `wa-verify:${wamid}`, providerMessageId: wamid, sentAt: new Date(),
            },
          });
        }
      }
  ...
  if (url.searchParams.get("logs") === "1") {
    out.logs = await db.whatsAppMessageLog.findMany({
      ...
      take: 5,
      select: { toPhone: true, status: true, error: true, sentAt: true, deliveredAt: true, readAt: true, failedAt: true },
    });
  }
